...
export default function LiquidationSection({
  ...
  // Obtener nombre del liquidador (el actual o el asignado)
  const liquidadorActual = allUsers?.find((u: any) => u.id === (data.analyst_liquidador || currentBill?.analyst_severance))?.full_name || 'Pendiente';
    ...
    <div className="space-y-6">
      ...
      <div className="bg-white border border-slate-200 rounded-xl shadow-sm p-8 space-y-7">
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
          ...
          <Input label="GNA" type="number" value={data.gna} onChange={(e) => handleInputChange('gna', e.target.value)} disabled={!canEdit} />
        </div>

        <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
          <Input label="Honorarios Médicos" type="number" value={data.honorarios_medic} onChange={(e) => handleInputChange('honorarios_medic', e.target.value)} disabled={!canEdit} />
          <Input label="Servicios Clínicos" type="number" value={data.servicios_clinicos} onChange={(e) => handleInputChange('servicios_clinicos', e.target.value)} disabled={!canEdit} />
          <div className="space-y-1">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
          ...
          <Input label="Nomenclatura de Lote *" value={data.nomenclature_pile} onChange={(e) => setData({...data, nomenclature_pile: e.target.value})} disabled={!canEdit} />
        </div>

        {/* --- SECCIÓN DEL ANALISTA (DENTRO DE LA CARD) --- */}
        <div className="mt-8 pt-6 border-t border-slate-100 flex justify-between items-end">
          ...
          <p className="text-[10px] text-slate-400 italic italic text-right max-w-[160px] leading-tight">
